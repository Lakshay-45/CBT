...
def run_titanic_eda():
    ...
    print("Standardized column names to lowercase.")
    print("Columns after loading & standardization:", titanic_data.columns.tolist())
    print("First 5 rows:\n", titanic_data.head())
    ...
    if titanic_data_filled['age'].isnull().any():
        ...
        try:
            # Select all numeric columns for context, but 'age' is the primary target for imputation
            numeric_cols_for_imputer = titanic_data_filled.select_dtypes(include=[np.number, bool]).columns.tolist()
            # Ensure 'age' is in the list if not already (it should be if float)
            ...
            # Apply imputer only to the slice of numeric columns
            imputed_values = imputer_age.fit_transform(titanic_data_filled[numeric_cols_for_imputer])
            # Assign back carefully, ensuring correct column alignment
            titanic_data_filled[numeric_cols_for_imputer] = pd.DataFrame(imputed_values, columns=numeric_cols_for_imputer, index=titanic_data_filled.index)
            print("'age' imputation complete using IterativeImputer.")
        except Exception as e:
    ...
    if 'cabin' in titanic_data_filled.columns and titanic_data_filled['cabin'].isnull().any():
        cabin_mode = titanic_data_filled['cabin'].mode()[0] # This will be one of the existing cabin values
        titanic_data_filled['cabin'].fillna(cabin_mode, inplace=True)
        print(f"Filled 'cabin' NaNs with mode: {cabin_mode} (Note: high missing % for cabin)")


    print("\nMissing values after imputation:\n", titanic_data_filled.isnull().sum()[titanic_data_filled.isnull().sum() > 0])
    ...
    for col, title in univariate_cols.items():
        if col in titanic_data_filled.columns:
            ...
            sns.countplot(x=col, data=titanic_data_filled)
            plt.title(title)
            if col == 'survived': plt.xticks([0,1], ['No', 'Yes'])
            plt.show()
            print(f"\n{title}:\n", titanic_data_filled[col].value_counts(dropna=False))

    for col_numeric in ['age', 'fare']:
        if col_numeric in titanic_data_filled.columns:
            plt.figure(figsize=(10,5))
            sns.histplot(x=col_numeric, data=titanic_data_filled, kde=True, bins=30 if col_numeric == 'age' else 50)
            plt.title(f'{col_numeric.capitalize()} Distribution (After Imputation)')
            plt.show()
            print(f"\n{col_numeric.capitalize()} Statistics (After Imputation):\n", titanic_data_filled[col_numeric].describe())
    ...
    for col in bivariate_vs_survived:
        if col in titanic_data_filled.columns and 'survived' in titanic_data_filled.columns:
            ...
            sns.countplot(x=col, hue='survived', data=titanic_data_filled)
            plt.title(f'Survival by {col.capitalize()}')
            plt.legend(title='Survived', labels=['No', 'Yes'])
            plt.show()
            print(f"\nSurvival by {col.capitalize()}:\n", pd.crosstab(titanic_data_filled[col], titanic_data_filled['survived']))

    if 'fare' in titanic_data_filled.columns and 'survived' in titanic_data_filled.columns:
        plt.figure(figsize=(10, 6))
        sns.histplot(titanic_data_filled, x='fare', hue='survived', bins=30, kde=True, multiple="stack", palette='viridis')
    ...
    if 'age' in titanic_data_filled.columns and 'survived' in titanic_data_filled.columns:
        ...
        titanic_data_filled['age_group'] = pd.cut(titanic_data_filled['age'], bins=age_bins, labels=age_labels, right=False)
        plt.figure(figsize=(8,5))
        sns.countplot(x='age_group', hue='survived', data=titanic_data_filled, order=age_labels)
        plt.title('Survival by Age Group')
        plt.legend(title='Survived', labels=['No', 'Yes'])
        plt.show()
        print("\nSurvival by Age Group:\n", pd.crosstab(titanic_data_filled['age_group'], titanic_data_filled['survived'], dropna=False))
    ...
    if 'sex' in titanic_data_filled.columns and 'pclass' in titanic_data_filled.columns and 'survived' in titanic_data_filled.columns:
        g = sns.catplot(x='sex', hue='survived', col='pclass', kind='count', data=titanic_data_filled, height=5, aspect=0.7)
    ...
    if 'sibsp' in titanic_data_filled.columns and 'parch' in titanic_data_filled.columns:
        ...
        if 'survived' in titanic_data_filled.columns:
            ...
            plt.show()
            print("\nSurvival by Family Size (% within size):\n",
                  pd.crosstab(titanic_data_filled['family_size'], titanic_data_filled['survived'], normalize='index') * 100)

            plt.figure(figsize=(6,4))
            sns.countplot(x='is_alone', hue='survived', data=titanic_data_filled)
            plt.title('Survival Based on Is Alone')
            plt.xticks([0,1], ['With Family', 'Alone'])
            plt.xlabel('Travel Status')
            plt.show()
            print("\nSurvival by Is Alone (% within status):\n",
                  pd.crosstab(titanic_data_filled['is_alone'], titanic_data_filled['survived'], normalize='index') * 100)
    ...
    if 'name' in titanic_data_filled.columns and 'survived' in titanic_data_filled.columns:
        ...
        sns.countplot(x='title', hue='survived', data=titanic_data_filled, order=title_order, palette='magma')
        plt.title('Survival Based on Title')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.show()
        print("\nSurvival by Title (% within title):\n",
              pd.crosstab(titanic_data_filled['title'], titanic_data_filled['survived'], normalize='index') * 100)
    else:
        print("WARNING: 'name' column not found or 'survived' column missing, cannot extract/analyze titles.")


    print_section_header("7. Correlation Heatmap")
    numeric_df_for_corr = titanic_data_filled.select_dtypes(include=np.number)
    # Drop PassengerId from correlation as it's just an identifier
    ...
    6. Key Conclusions & Insights:
       ...
       - Limitations: High missingness in `cabin` restricts its analytical value despite imputation.
